...
{
public:
	FCsvWriterHelper(const TSharedRef<FArchive>& InOutputFile, int32 InBufferSize)
...
FString GetPathName()
{
	...
	const FString DestinationFolder = FPaths::ProfilingDir() + FString::Printf(TEXT("%s-%d/"), *FDateTime::Now().ToString(TEXT("%Y%m%d_%H%M%S")), ++Counter);
	return DestinationFolder;
}

FString VectorToString(const FVector& Vec)
{
	return FString::Printf(TEXT("X=%.3f,Y=%.3f,Z=%.3f"), Vec.X, Vec.Y, Vec.Z);
}

typedef TArray<FVector> FParticleDatas;
typedef TArray<FParticleDatas> FParticleDatasSeq;
FParticleDatasSeq ParticleLinearVels;
FParticleDatasSeq ParticleAngularVels;
FParticleDatasSeq ParticleLocs;

void WriteCSV()
{
	const int32 BufferSize = 128 * 1024; // 128 KB

	// Linear Velocity
	const FString PathName = GetPathName();
	const FString LinearVelocityFileName = PathName + TEXT("LinearVelocity.csv");
	TSharedPtr<FArchive> LinearVelFile = MakeShareable(IFileManager::Get().CreateFileWriter(*LinearVelocityFileName));
	FCsvWriterHelper CsvWriter_LinearVel(LinearVelFile.ToSharedRef(), BufferSize);
	for (FParticleDatasSeq::TConstIterator Iter = ParticleLinearVels.CreateConstIterator(); Iter; ++Iter)
	{
		const FParticleDatas& ParticleDatas = *Iter;
		for (FParticleDatas::TConstIterator Iter2 = ParticleDatas.CreateConstIterator(); Iter2; ++Iter2)
		{
			const FVector& Velocity = *Iter2;
			CsvWriter_LinearVel.WriteString(VectorToString(Velocity));
		}
		CsvWriter_LinearVel.NewLine();
	}

	// Angular Velocity
	const FString AngularVelocityFileName = PathName + TEXT("AngularVelocity.csv");
	TSharedPtr<FArchive> AngularVelFile = MakeShareable(IFileManager::Get().CreateFileWriter(*AngularVelocityFileName));
	FCsvWriterHelper CsvWriter_Angular(AngularVelFile.ToSharedRef(), BufferSize);
	for (FParticleDatasSeq::TConstIterator Iter = ParticleAngularVels.CreateConstIterator(); Iter; ++Iter)
	{
		const FParticleDatas& ParticleDatas = *Iter;
		for (FParticleDatas::TConstIterator Iter2 = ParticleDatas.CreateConstIterator(); Iter2; ++Iter2)
		{
			const FVector& Velocity = *Iter2;
			CsvWriter_Angular.WriteString(VectorToString(Velocity));
		}
		CsvWriter_Angular.NewLine();
	}

	// Location
	const FString LocationFileName = PathName + TEXT("Locaiton.csv");
	TSharedPtr<FArchive> LocationFile = MakeShareable(IFileManager::Get().CreateFileWriter(*LocationFileName));
	FCsvWriterHelper CsvWriter_Location(LocationFile.ToSharedRef(), BufferSize);
	for (FParticleDatasSeq::TConstIterator Iter = ParticleLocs.CreateConstIterator(); Iter; ++Iter)
	{
		const FParticleDatas& ParticleDatas = *Iter;
		for (FParticleDatas::TConstIterator Iter2 = ParticleDatas.CreateConstIterator(); Iter2; ++Iter2)
		{
			const FVector& Loc = *Iter2;
			CsvWriter_Location.WriteString(VectorToString(Loc));
		}
		CsvWriter_Location.NewLine();
	}
}
// end:@minyul
...
void UDestructibleComponent::BeginPlay()
{
	...
	ParticleLinearVels.Empty();
	ParticleAngularVels.Empty();
	ParticleLocs.Empty();
...
void UDestructibleComponent::SetChunksWorldTM(const TArray<FUpdateChunksInfo>& UpdateInfos)
{
	...
	// begin:@minyul
	FParticleDatas& LinearVelocities = ParticleLinearVels.AddDefaulted_GetRef();
	FParticleDatas& AngularVelocities = ParticleAngularVels.AddDefaulted_GetRef();
	FParticleDatas& Locations = ParticleLocs.AddDefaulted_GetRef();
	// end:@minyul	
	...
	for (const FUpdateChunksInfo& UpdateInfo : UpdateInfos)
	{
		...
		// begin:@minyul
		LinearVelocities.Add(UpdateInfo.LinearVelocity);
		AngularVelocities.Add(UpdateInfo.AngularVelocity);
		Locations.Add(WorldTranslation);		
		// end:@minyul
